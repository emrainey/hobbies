...
TEST(ImageTest, IteratorsTest) {
    ...
    img3.generate_each([&](const image::point&) -> color {
        return colors::red;
    });
...
TEST(ImageTest, SubsamplingCount) {
    ...
    size_t counter = 0;
    img.generate_each([&](const image::point&) -> color {
        counter++;
...
TEST(ImageTest, SubsamplingChecker) {
    ...
    image5.generate_each([&](const image::point&) -> color {
        return colors::white;
    });
    image image6(24, 32);
    double ratio = (image5.height / image6.height);
    image6.generate_each([&](const image::point& p) -> color {
