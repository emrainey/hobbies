// g++ -O3 -g -std=c++20 -Wall -Werror -Wextra -pedantic -o ecs ecs.cpp -I/opt/homebrew/include/SDL -L/opt/homebrew/lib -lsdl && ./ecs
#include <cstdint>
#include <cstddef>
#include <memory>
#include <limits>
...
public:
    ...
    using Type = TYPE;
    Manager() : next_available_{0U}, items_{}, valid_{} {} // must put all elements into a known state
...
//        } u;
        ...
        window = SDL_CreateWindow("ECS Particles",
...
//            std::cout << "Now: " << std::fixed << std::setprecision(7) << now << " =0b" << b << std::endl;
            ...
            if (delta > diff_us) {
                ...
                error += increment;
                std::cout << "Diff was " << diff_us.count() << ", delta=" << delta.count() << " error=" << error.count() << std::endl;
