...
namespace raytrace {
namespace objects {
/// A cylinder with an outward facing normal and end caps
class capped_cylinder : public object {
public:
    /// Constructs a cylinder which is up and down in Z and infinite in height.
    capped_cylinder(point const& C, precision radius);
    /// Constructs a cylinder which is up and down in Z.
    /// The point C is in the center of the cylinder between the top and bottom.
    capped_cylinder(point const& C, precision half_height, precision radius);
    ...
    capped_cylinder(point const& base, point const& apex, precision radius);

    virtual ~capped_cylinder() = default;
