...
* `test/` - Additional tests for the project, often used for integration or end-to-end tests.
...
- ✅ **Always do** When generating commit messages be clear what code is generated by you, the agent, and what was not. Always state which model and token window size was used.
- ✅ **Always do** Run a full build before pushing a commit to ensure that all changes are correctly integrated and no build errors are present.
