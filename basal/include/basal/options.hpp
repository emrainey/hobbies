...
void print(config (&options)[NUM_OPTS]) {
    ...
    for (size_t j = 0; j < NUM_OPTS; j++) {
        if (std::holds_alternative<bool>(options[j].value)) {
            printf("\t%3s, %20s (default: %s) desc: %s\n", options[j].short_switch.c_str(), options[j].long_switch.c_str(), std::get<bool>(options[j].value) ? "true" : "false", options[j].description.c_str());
            ...
            printf("\t%3s, %20s (default: %s) desc: %s\n", options[j].short_switch.c_str(), options[j].long_switch.c_str(), std::get<std::string>(options[j].value).c_str(), options[j].description.c_str());
        }
    }
}
