...
protected:
    ...
    static_assert(std::is_floating_point<typename pack_type::element_type>::value, "Must be either float, double or long double");
    static_assert(2 <=pack_type::number_of_elements, "Must be a Intel Parallel/Multiple Data data type");
    static_assert(dimensions <= pack_type::number_of_elements, "The dimension of elements must fit into the number of parallel data slots");
...
public:
    ...
    xmm_& operator^=(typename pack_type::element_type a) {
        for (auto& d : pack_type::datum) { d = std::pow(d, a); }
        return (*this);
    }

    /** Allows each element to be changed */
    using alteration_iterator = std::function<typename pack_type::element_type(size_t, typename pack_type::element_type)>;
