...
point Bounds::center() const {
    if (not is_infinite()) {
        ...
        if (std::isinf(min.x()) and std::isinf(min.y()) and std::isinf(min.z()) and std::isinf(max.x())
            and std::isinf(max.y()) and std::isinf(max.z())) {
            return R3::origin;
        }
...
bool Bounds::is_infinite() const {
    return (std::isinf(min.x()) or std::isinf(min.y()) or std::isinf(min.z()) or std::isinf(max.x())
            or std::isinf(max.y()) or std::isinf(max.z()));
}
...
std::vector<Bounds> Bounds::split() const {
    ...
    std::array<point, NumSubBounds> sub_min{point{min.x(), min.y(), min.z()}, point{min.x(), min.y(), mid.z()},
                                            point{min.x(), mid.y(), min.z()}, point{min.x(), mid.y(), mid.z()},
                                            point{mid.x(), min.y(), min.z()}, point{mid.x(), min.y(), mid.z()},
                                            point{mid.x(), mid.y(), min.z()}, mid};
