...
// New tests for capped cylinder
TEST(CappedCylinderTest, Type) {
    using namespace raytrace;
    using namespace raytrace::objects;
    capped_cylinder cyl1{R3::origin, 1.0_p, 1.0_p};
    ASSERT_EQ(cyl1.get_type(), Type::Cylinder);
    ASSERT_TRUE(cyl1.has_definite_volume());
}

TEST(CappedCylinderTest, Normals) {
    using namespace raytrace;
    using namespace raytrace::objects;

    capped_cylinder cyl1(R3::origin, 1.0_p, 1.0_p);

    raytrace::point p0{1, 0, 1};  // Top cap
    raytrace::point p1{0, 1, 1};  // Top cap
    raytrace::point p2{-1, 0, -1}; // Bottom cap
    raytrace::point p3{0, -1, -1}; // Bottom cap
    raytrace::point p4{1, 0, 0};  // Side surface
...
TEST(CappedCylinderTest, RayIntersection) {
    using namespace raytrace;
    using namespace raytrace::objects;
    ...
    {
        // Test cap intersection
        raytrace::point p{0, 0, 2};
        ray r{p, -R3::basis::Z};
        raytrace::point i{0, 0, 1};
        raytrace::objects::object::hit h = cyl1.intersect(r);
        ASSERT_EQ(geometry::IntersectionType::Point, get_type(h.intersect));
        ASSERT_POINT_EQ(i, as_point(h.intersect));
    }
    {
        // Test bottom cap intersection
        raytrace::point p{0, 0, -2};
        ray r{p, R3::basis::Z};
        raytrace::point i{0, 0, -1};
        raytrace::objects::object::hit h = cyl1.intersect(r);
...
TEST(CappedCylinderTest, Mapping) {
    using namespace raytrace;
    using namespace raytrace::objects;

    for (precision h2 = 1.0_p; h2 < 10.0_p; h2 += 1.0_p) {
        for (precision r = 1.0_p; r < 10.0_p; r += 1.0_p) {
            capped_cylinder cyl1(R3::origin, h2, r);
            {
                raytrace::point p{r, 0, 0};
                image::point uv(0.5_p, 0.5_p);
                image::point tmp = cyl1.map(p);
