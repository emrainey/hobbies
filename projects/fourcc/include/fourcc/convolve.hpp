...
void sobel_mask(image<PixelFormat::IYU2> const& iyu2_image,
                image<PixelFormat::Y8>& mask);

/// Produces a Sobel Mask image
void sobel_mask(image<PixelFormat::RGB8> const& rgb_image,
                image<PixelFormat::Y8>& mask);

/// Runs a 1D filter across a linear uncompressed image
/// @warning no gamma correction!
void filter(image<PixelFormat::RGBf>& output,
            image<PixelFormat::RGBf> const& input, float const kernel[3]);

/// Runs a 1D filter across a linear uncompressed image
/// @warning no gamma correction!
void filter(image<PixelFormat::RGBId>& output,
            image<PixelFormat::RGBId> const& input, double const kernel[3]);

/// Runs a 1D filter across an encoded image
/// @warning no gamma correction!
void filter(image<PixelFormat::RGB8>& output,
            image<PixelFormat::RGB8> const& input, int16_t const kernel[3]);
...
/// Runs a 2D 3x3 Filter across an image
/// @warning no gamma correction!
void filter(image<PixelFormat::RGBId>& output, image<PixelFormat::RGBId> const& input,
            double (&kernel)[3][3]);
