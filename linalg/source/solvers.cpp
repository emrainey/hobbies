...
std::tuple<element_type, element_type, element_type> cubic_roots(element_type a, element_type b, element_type c, element_type d) {
    ...
    basal::exception::throw_if(std::isnan(x1) and std::isnan(x2) and std::isnan(x3), __FILE__, __LINE__, "Cubics always have at least 1 solution");
    return std::make_tuple(x1, x2, x3);
}

std::tuple<element_type, element_type, element_type, element_type>
quartic_roots(element_type a, element_type b, element_type c, element_type d, element_type e) {
    using namespace std::literals::complex_literals;
    statistics::get().quartic_roots++;
    if constexpr (root_debug) {
        std::cout << "Quartic Coefficients: a=" << a << ", b=" << b << ", c=" << c << ", d=" << d << ", e=" << e << std::endl;
    }
    if (basal::equals_zero(a)) {
