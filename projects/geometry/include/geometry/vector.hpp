...
/// @tparam DIMS The number of dimensions, must be between 2 and 4 (inclusive)
...
public:
    ...
    /// The constructor from a pointer to an array
    vector_(size_t len, precision in[]);

    /// The initializer list constructor, should not be inferred.
    vector_(std::initializer_list<precision> a);

    /// Copy Constructor
    vector_(vector_ const& other);

    /// Move Constructor
    vector_(vector_&& other);
    ...
    void print(std::ostream& os, char const name[]) const;

    /// Returns the dot product of two vectors. Reference parameter form
    friend inline precision dot(vector_ const& a, vector_ const& b) noexcept(false) {
        precision d = 0.0_p;
        for (size_t i = 0; i < dimensions; i++) {
            d += (a.data_[i] * b.data_[i]);
        }
    ...
    friend inline precision spread(vector_ const& a, vector_ const& b) noexcept(false) {
    ...
    friend inline bool operator==(vector_ const& a, vector_ const& b) {
        for (size_t i = 0; i < dimensions; i++) {
            if (not basal::nearly_equals(a.data_[i], b.data_[i])) {
                return false;
            }
        }
        return true;
    ...
    friend inline vector_ multiply(matrix const& A, vector_ const& b) {
    ...
    friend inline vector_ negation(vector_ const& a) {
        vector_ b{a};  // copy
        b *= -1.0_p;   // scale
        return b;      // return
    }
