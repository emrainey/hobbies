...
TEST(LinalgExtraTests, QuarticRoots04) {
    ...
    EXPECT_QUAD_TUPLE_EQ4(3.7320508075688767,
                          0.26794919243112258,
                          1.7320508075688774,
                          -1.7320508075688767, roots);
...
#define EXPECT_QUAD_TUPLE_EQ1(A, tup) { \
    ...
    if (not std::isnan(x1)) { EXPECT_NEAR(A, x1, 1E-6); } \
    if (not std::isnan(x2)) { EXPECT_NEAR(A, x2, 1E-6); } \
    if (not std::isnan(x3)) { EXPECT_NEAR(A, x3, 1E-6); } \
    if (not std::isnan(x4)) { EXPECT_NEAR(A, x4, 1E-6); } \
}
