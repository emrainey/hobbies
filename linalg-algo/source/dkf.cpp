...
matrix dkf::prediction(matrix& u) {
    basal::exception::throw_unless(u.rows == ni && u.cols == 1, __FILE__, __LINE__,
                                   "Inputs must have the dimensions described in constructor, inputs x 1");
    x[k] = A * x[k_1] + B * u;
    P[k] = (A * P[k_1]) * (A ^ T) + Q;
    return x[k];
}
...
matrix dkf::correction(matrix& z) {
    basal::exception::throw_unless(
        z.rows == nm && z.cols == 1, __FILE__, __LINE__,
        "Measurements must have the dimensions described in the constructor, measurements x 1");
