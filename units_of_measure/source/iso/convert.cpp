...
// radians <=> turns
radians convert(turns &A) { return radians(A.value * tau); }
turns convert(radians &A) { return turns(A.value / tau); }

// seconds <=> cycles
hertz convert(seconds &A) { return hertz(1.0/A.value); }
seconds convert(hertz &B) { return seconds(1.0/B.value); }
