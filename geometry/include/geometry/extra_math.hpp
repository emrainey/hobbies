...
namespace geometry {
    ...
    matrix rowjoin(matrix& a, vector_<DATA_TYPE, DIMS>& b) noexcept(false) {
        basal::exception::throw_unless(a.rows == b.dimensions, __FILE__, __LINE__, "To join, the number of rows must be equal");
    ...
    matrix span(const std::vector<geometry::vector_<DATA_TYPE, DIM>> &set) noexcept(false) {
        ...
        for (auto& v : set) {
            basal::exception::throw_if(v.dimensions == m.rows, __FILE__, __LINE__, "Each vector must match dimensionality for the first vector.");
        }
