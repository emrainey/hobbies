...
// Rate template!
template <typename NumeratorType, typename DenominatorType> class rate {
static_assert(not std::is_same<NumeratorType, DenominatorType>::value, "Must be different types!");
static_assert(std::is_same<typename NumeratorType::type, typename DenominatorType::type>::value, "Unit types must be the same!");
public:
...
public:
    ...
    rate(const rate& other)
        : _value(other.value), value(_value)
