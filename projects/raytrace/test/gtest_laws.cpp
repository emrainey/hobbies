...
TEST(LawsTest, SnellsNormalIncident) {
    ...
    vector R = laws::snell(N, I, 1.0_p, 1.0_p).normalize();
    ASSERT_VECTOR_EQ(I, R);
    R = laws::snell(N, I, 1.0_p, 2.0_p).normalize();
    ASSERT_VECTOR_EQ(I, R);
}
...
TEST(LawsTest, SnellsTotalInternalReflection) {
    ...
    // Should pass through identically
    vector R = laws::snell(N, I, 2.0_p, 2.0_p).normalize();
    ASSERT_VECTOR_EQ(I, R);

    // Should be TIR
    R = laws::snell(N, I, 2.0_p, 1.0_p);
...
TEST(LawsTest, Fresnel) {
    ...
    ASSERT_NEAR(iso::pi / 4.0_p, incident_angle.value, basal::epsilon);
    ...
    precision Rr = 1.0_p - Rf;
    precision Rc = 1.0_p - 0.5_p * (0.203777_p + 0.0415249_p);  // numbers from calc
    ASSERT_NEAR(Rc, Rr, 1E-5);  // should be accurate to 5 decimal places since the numbers are from a float calculator
}

TEST(LawsTest, Beers) {
    vector D{{8, 0, 0}};
    precision dropoff = laws::beers(D, 0.186_p);
    ASSERT_NEAR(0.226_p, dropoff, 1E-2);

    // at no distance the dropoff coefficient should be 1.0_p unity
    dropoff = laws::beers(geometry::R3::null, 0.186_p);
    ASSERT_PRECISION_EQ(1.0_p, dropoff);
}

TEST(LawsTest, InverseLaw) {
    ASSERT_PRECISION_EQ(1.0_p, laws::inverse_square(0.0_p));
    ASSERT_PRECISION_EQ(0.64_p, laws::inverse_square(0.25_p));
    ASSERT_PRECISION_EQ(0.25_p, laws::inverse_square(1.0_p));
    ASSERT_PRECISION_EQ(0.0625_p, laws::inverse_square(3.0_p));
}
...
TEST(LawsTest, Lambertian) {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        ASSERT_NEAR(1.0_p, R.magnitude(), basal::epsilon);
    }
}
...
TEST(LawsTest, PenetrationDepth) {
    ...
    precision permittivity = -5.0_p * 8.885E-12;   // Farads per meter
