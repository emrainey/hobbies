...
line::line(const R3::vector &ov, const R3::point &op)
    : m_udir(ov)
    , m_zero(op) {
    basal::exception::throw_if(m_udir.is_zero(), __FILE__, __LINE__, "Vector can not be zero length");
}

line::line(const R3::point &op, const R3::vector &ov)
    : m_udir(ov)
    , m_zero(op) {
    basal::exception::throw_if(m_udir.is_zero(), __FILE__, __LINE__, "Vector can not be zero length");
}

line::line(const R3::point &a, const R3::point &b)
    : m_udir(a - b)
    , m_zero(b) {
...
line::line(const std::vector<element_type> &list)
    : m_udir{{list[0], list[1], list[2]}}
    , m_zero(list[3],list[4],list[5]) {
...
bool line::solve(const point &P, element_type &t) const {
    ...
    if (std::all_of(ts.begin(), ts.end(), [&](element_type v) {
        return basal::equals(v, ts[0]);
    })) {
        t = ts[0];
        return true;
    }
...
bool operator==(const line &a, const line &b) {
    ...
    if ((a.direction() == b.direction()) and
        (geometry::operators::operator==(a.position(), b.position()))) {
        return true;
    }
    // equal lines means
